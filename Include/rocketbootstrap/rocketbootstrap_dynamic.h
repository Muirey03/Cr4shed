...
static kern_return_t rocketbootstrap_look_up(mach_port_t bp, const name_t service_name, mach_port_t *sp)
{
	...
	if (!impl) {
		...
		if (handle)
			impl = (kern_return_t (*)(mach_port_t bp, const name_t service_name, mach_port_t *sp))dlsym(handle, "rocketbootstrap_look_up");
		if (!impl)
...
static kern_return_t rocketbootstrap_unlock(const name_t service_name)
{
	...
	if (!impl) {
		void *handle = dlopen("/usr/lib/librocketbootstrap.dylib", RTLD_LAZY);
		if (handle)
			impl = (kern_return_t (*)(const name_t service_name))dlsym(handle, "rocketbootstrap_unlock");
		if (!impl)
			return -1;
	}
...
static kern_return_t rocketbootstrap_register(mach_port_t bp, name_t service_name, mach_port_t sp)
{
	...
	if (!impl) {
		void *handle = dlopen("/usr/lib/librocketbootstrap.dylib", RTLD_LAZY);
		if (handle)
			impl = (kern_return_t (*)(mach_port_t bp, name_t service_name, mach_port_t sp))dlsym(handle, "rocketbootstrap_register");
		if (!impl)
...
static CFMessagePortRef rocketbootstrap_cfmessageportcreateremote(CFAllocatorRef allocator, CFStringRef name)
{
	...
	if (!impl) {
		void *handle = dlopen("/usr/lib/librocketbootstrap.dylib", RTLD_LAZY);
		if (handle)
			impl = (CFMessagePortRef (*)(CFAllocatorRef allocator, CFStringRef name))dlsym(handle, "rocketbootstrap_cfmessageportcreateremote");
		if (!impl)
...
static kern_return_t rocketbootstrap_cfmessageportexposelocal(CFMessagePortRef messagePort)
{
	...
	if (!impl) {
		void *handle = dlopen("/usr/lib/librocketbootstrap.dylib", RTLD_LAZY);
		if (handle)
			impl = (kern_return_t (*)(CFMessagePortRef messagePort))dlsym(handle, "rocketbootstrap_cfmessageportexposelocal");
		if (!impl)
			return -1;
	}
...
static void rocketbootstrap_distributedmessagingcenter_apply(CPDistributedMessagingCenter *messaging_center)
{
	...
	if (!impl) {
		void *handle = dlopen("/usr/lib/librocketbootstrap.dylib", RTLD_LAZY);
		if (handle)
			impl = (void (*)(CPDistributedMessagingCenter *messagingCenter))dlsym(handle, "rocketbootstrap_distributedmessagingcenter_apply");
		if (!impl)
			return;
	}
